...
from owdfa.algorithm.ssl_CPL import SSLModel
...
class MPSL(SSLModel):
    def __init__(self, args):
        ...
        self.eta4 = args.method.MPSL.eta4
        self.eta5 = args.method.MPSL.eta5
        self.batch_size = args.train.batch_size

    def get_loss_names(self):
        loss_name = ['total_loss', 'ce_loss',
                     'mpv_loss', 'fcp_loss', 'en_loss', 'aux_loss', 'match_loss']
        return loss_name
    ...
    def aux_loss(self, y_pred_raw, y_pred_aux, target):
        y_pred_cal = y_pred_raw - y_pred_aux
        aux_loss = self.ce(y_pred_raw[:len(target)], target)+ \
                     self.ce(y_pred_cal[:len(target)], target)
        
        y_prob_aux = F.softmax(y_pred_aux[:len(target)], dim=1)
        cal_loss = F.cross_entropy(y_pred_aux[:len(target)], y_prob_aux)
    ...
    def loss(self, **kwargs):
        ...
        y_pred_raw = kwargs['y_pred_raw']
        y_pred_aux = kwargs['y_pred_aux']
        y_aug_raw = kwargs['y_aug_raw']
        y_aug_aux = kwargs['y_aug_aux']
        match_loss = kwargs['match_loss']
        target = kwargs['target']
        x = kwargs['x']
        ...
        # aux_loss
        aux_loss1 = self.aux_loss(y_pred_raw, y_pred_aux, target)
        aux_loss2 = self.aux_loss(y_aug_raw, y_aug_aux, target)
        aux_loss = aux_loss1 + aux_loss2
        loss_map['aux_loss'] = aux_loss
        loss_map['match_loss'] = match_loss
        # Total loss
        total_loss = ce_loss + self.eta1 * mpv_loss + \
            self.eta2 * fcp_loss - self.eta3 * en_loss + aux_loss * self.eta4 + match_loss * self.eta5
