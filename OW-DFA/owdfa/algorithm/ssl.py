...
class SSLModel(SLModel):
    ...
    def training_step(self, batch, batch_idx):
        ...
        output, y_pred_raw, y_pred_aux, y_aug_raw, y_aug_aux, match_loss, (f_g, f_p) = self.encoder(x)

        # Calculate the loss using the loss function
        loss_map = self.loss(labeled_len=labeled_len, total_len=total_len, output=output, y_pred_raw = y_pred_raw, y_pred_aux = y_pred_aux,
                            y_aug_raw = y_aug_raw, y_aug_aux = y_aug_aux, match_loss = match_loss,
                             feat=f_g, feat_p=f_p, target=target, target_ulb=idx_ulb, x=x, y=y)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        output, _, _, _, _, _, _ = self.encoder(image)
