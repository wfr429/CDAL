...
class PGTrainer(): 
    ...
    def aux_loss(self, input_fea, y_pred_raw, y_pred_aux, target):
        y_pred_cal = y_pred_raw - y_pred_aux
        _, aux_loss1 = self.criterion(input_fea, y_pred_raw, target)
        _, aux_loss2 = self.criterion(input_fea, y_pred_cal, target)
        aux_loss = aux_loss1 + aux_loss2

        y_prob_aux = F.softmax(y_pred_aux, dim=1)
        cal_loss = F.cross_entropy(y_pred_aux, y_prob_aux)

        return aux_loss - cal_loss
    ...
    def train_epoch_POSE(self, augnets, epoch):
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            _, aug_fea, _, _, _, _, _ = self.model(aug_img, data=self.config.input_data)
            input_prob, input_fea, _, _, _, _, _ = self.model(input_img, data=self.config.input_data)
            ...
            if len(augnets) >= 1:
                ...
                _, aug_fea_pre, _, _, _, _, _ = self.model(aug_img_pre, data=self.config.input_data)
            ...
            input_prob, input_fea, y_pred_raw, y_pred_aux, y_aug_raw, y_aug_aux, match_loss = self.model(input_img, data=self.config.input_data)
            input_prob, loss_cls = self.criterion(input_fea, input_prob, label)
            aux_loss = self.aux_loss(input_fea, y_pred_raw, y_pred_aux, label)
            aug_loss = self.aux_loss(input_fea, y_aug_raw, y_aug_aux, label)
            add_loss = aux_loss*0.5 + aug_loss*0.5 + match_loss*0.2
            ...
            loss = loss_cls + loss_metric + add_loss
    ...
    def tsne_augnet(self, epoch, augnets, dataloader, run_type='tsne'):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(dataloader):
                ...
                _, feature, _, _, _, _, _ = self.model(input_img, data=self.config.input_data)
                _, aug_feature, _, _, _, _, _ = self.model(aug_imgs, data=self.config.input_data)
    ...
    def predict_set(self, dataloader, run_type='test'): 
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(dataloader):
                ...
                prob, feature, _, _, _, _, _ = self.model(input_img, data=self.config.input_data)
